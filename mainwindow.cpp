    ...
    : QMainWindow(parent) , ui(new Ui::MainWindow)
{
    ...
    this->DB.setDatabaseName("storage.db");
    if(this->DB.open()){
       this->query = QSqlQuery(this->DB);
       this->query.exec("CREATE TABLE IF NOT EXISTS calc (result text)");
    }
}
...
void MainWindow::on_pushButton_res_clicked()
{
    ...
    if(this->DB.open())
     this->query.exec("INSERT INTO calc VALUES (" + new_label + ")");
}
...
{
    ui->listWidget->clear();
    this->query.exec("SELECT result FROM calc");
    this->addValues();

    this->query.exec("DELETE FROM calc");
}
